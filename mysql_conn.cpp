...
int mysql_conn::query_sql(const std::string sql, MySQL_Row_Set &row_set, std::string &errInfo) {
    ...
    if (!_conn) {
        printf("the mysql is not connected.\n");
        errInfo = "the mysql is not connected";
        return -1;
    }

    if (sql.empty()) {
        printf("the sql is empty.\n");
        errInfo = "the sql is empty";
        return -1;
    }

    printf("excute sql command:%s.\n", sql.c_str());
    nRet = mysql_real_query(_mysql, sql.c_str(), sql.size());
    if (0 != nRet) {
        errInfo = mysql_error(_mysql);
        printf("mysql_real_query failed, sql:%s, ret:%d, err code:%d, err msg:%s\n",
    ...
    if (!pRes) {
        printf("mysql_store_result failed, sql:%s, err code:%d, err msg:%s\n",
    ...
    if (0 == num_row) {
        ...
        // return -5;
        printf("query success but get zero record.\n");
        return 0;
    }
    ...
    printf("query sucess. get %d record\n", num_row);
    return 0;
}
...
int mysql_conn::execute_sql(const std::string sql, unsigned long long &last_insert_id, unsigned long long &affected_rows, std::string &errInfo) {
    ...
    if (0 != nRet) {
        errInfo = mysql_error(_mysql);
        printf("execute_sql failed, ret:%d, err code:%d, err msg:%s\n",
    ...
    */
    ...
    printf("excute sql command success, last insert id:%d, affected rows:%d.\n",
...
void mysql_conn::release_conn() {
    if (_conn) {
        ...
        printf("the mysql isn't connect, can't release it now.\n");
    }
}
...
bool mysql_conn::ping() {
    ...
    */
    ...
    if (nRet != 0) {
        printf("mysql server isn't connectd, mysql_ping failed, ip:%s, port:%d, user:%s, pwd:%s, "
...
int mysql_conn::autocommit(bool open) {
    ...
    if (0 != nRet) {
        printf("mysql_autocommit failed, ret:%d, err code:%d, err msg%s\n", nRet, mysql_errno(_mysql), mysql_error(_mysql));
    }
...
int mysql_conn::commit() {
    ...
    if (0 != nRet) {
        printf("mysql_commit failed, ret:%d, err code:%d, err msg%s\n", nRet, mysql_errno(_mysql), mysql_error(_mysql));
    }
...
int mysql_conn::rollback() {
    ...
    if (0 != nRet) {
        printf("mysql_rollback failed, ret:%d, err code:%d, err msg%s\n", nRet, mysql_errno(_mysql), mysql_error(_mysql));
    }
